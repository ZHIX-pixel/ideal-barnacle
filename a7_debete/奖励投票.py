...
def process_sql_generation2(user_question, sql_query, Professional_vocabulary, user_question_type,
    ...
    while iteration < max_iterations:
        ...
        try:
            # 生成SQL提示
            prompt_t = f"""
            您是一位Text-to-SQL专家，擅长对SQL语句进行打分。请根据为下面两个SQL语句根据用户问题投票，最后输出投票后的SQL
            用户问题：{user_question}；
            相关术语：{Professional_vocabulary}；
            用户问题查询类型：{user_question_type}；
            数据库结构：{class_names}；
