...
import a00_model.main as a1
# 错误定位器
def check_syntax(cursor, sql_query, Professional_vocabulary, user_question, user_question_type, class_names, similar_records):
    try:
        ...
        prompt_t = f"""
           ...
           """
        # 获取SQL响应
        zhipu_AI_out = a1.get_response_from_model_1(prompt_t)
        ...
        if result != "":  # 如果有错误
            ...
            if "未发现 SQL 语义错误" in result:
                ...
                return check_syntax(cursor, sql_query1, Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
        else:
...
def repair_program(cursor,sql_query,error_information,Professional_vocabulary, user_question, user_question_type, class_names, similar_records):
    ...
    """
    # 获取SQL响应
    user_question_duo = a1.get_response_from_model_2(prompt_t)
    print("-------------------------------------------------------")
    ...
    """
    # 获取SQL响应
    sql_query = a1.get_response_from_model_22(prompt_t)
    print("-------------------------------------------------------")
...
def Repair_serum(cursor,Candidate_result,Professional_vocabulary, user_question, user_question_type, class_names, similar_records):
    ...
    """
    # 获取SQL响应
    sql_query=a1.get_response_from_model_3(prompt_t)
    print("-------------------------------------------------------")
...
def Suggestion_optimizer(cursor, sql_query, user_question, class_names,i):
    ...
    # 获取SQL响应
    sql_query11 = a1.get_response_from_model_4(prompt_t)
    print("-------------------------------------------------------")
...
# sql_query,Professional_vocabulary, user_question, user_question_type, class_names, similar_records = a11.generate_sql_model1(user_question,model,client)
# sql_query =extract_sql_query(sql_query)
...
#
# final_sql1 = check_syntax(cursor, sql_query, Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
# final_sql = ""
...
#     final_sql2 = check_syntax(cursor, final_sql2, Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
#     # 第二次检查
#     if final_sql2[:2] == '对的':
...
#         final_sql3 = check_syntax(cursor, final_sql3, Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
#         if final_sql3[:2] == '对的':
...
def generate_sql_and_check1(user_question, model, client, cursor):
    """生成并检查 SQL 查询"""
    # 生成SQL查询
    sql_query, Professional_vocabulary, user_question, user_question_type, class_names, similar_records = a11.generate_sql_model1(user_question, model, client)
    sql_query = extract_sql_query(sql_query)
    ...
    if final_sql1[:2] == '对的':
        ...
        # final_sql2 = final_sql1[2:]
        final_sql = final_sql1
        final_sql2 = check_syntax(cursor, final_sql1[2:], Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
        ...
        if final_sql2[:2] == '对的':
            ...
            final_sql = final_sql2
            # final_sql3 = final_sql2[2:]
            final_sql3 = check_syntax(cursor, final_sql2[2:], Professional_vocabulary, user_question, user_question_type, class_names, similar_records)
            if final_sql3[:2] == '对的':
...
# cursor = connection.cursor()
# client = weaviate.Client("http://localhost:8080")
# user_question = "获取“material_appropriation”表中所有类型为“借用”的记录。"
# model = SentenceTransformer('D:/my_langchain/myProject1/text2vec-base-chinese')
# a=generate_sql_and_check1(user_question, model, client, cursor)
# print(a)
# # 关闭数据库连接
# cursor.close()
# connection.close()